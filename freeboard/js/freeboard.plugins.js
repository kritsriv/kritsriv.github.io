...
(function () {
	...
	var zetaDatasource = function (settings, updateCallback) {
		var self = this;
		var updateTimer = null;
		var currentSettings = settings;
		var errorStage = 0; 	// 0 = try standard request
		...
		// 		redirect: 'follow'
		// 	};
		// 	var signal = CryptoJS.HmacSHA1(currentSettings.api_key, currentSettings.secret);
		// 	var response = await fetch("http://35.176.36.141:25455/teamcms/ws/auth_v1/auth_token/query/getWanAccessToken?api_key=65296608431940f190e15665063d1bb3&signal=" + signal.toString(), requestOptions);
		// 	var obj = await response.json();
		// 	var access_token = obj.data[0].access_token;
  		// 	return access_token;
		// }
		...
		function getencodeBody() {
			// Get current Unix timestamp in milliseconds
			// Convert milliseconds to seconds
			//const starttime = Math.floor(new Date.setHours(new Date.getHours() - (currentSettings.hours)? currentSettings.hours : 24) / 1000);
			var starttime =  Math.floor((Date.now() / 1000)- (((currentSettings.hours)? currentSettings.hours : 24) * 3600));
			...
			var key = CryptoJS.enc.Utf8.parse(currentSettings.secret);
			var iv = CryptoJS.enc.Utf8.parse('123456789abcdef0');
			...
			var array = [];
			var array1 = cryptArrToByteArr(iv);
			var array2 = cryptArrToByteArr(encrypted.ciphertext);
			...
			//var array = [...cryptArrToByteArr(iv), ...cryptArrToByteArr(encrypted.ciphertext)];
			//var array2 = cryptArrToByteArr(encrypted.ciphertext);
			//var array = [].concat(array1,array2);
			return _arrayBufferToBase64(array);
		...
		this.updateNow = function () {
			...
			fetch("http://35.176.36.141:25455/teamcms/ws/auth_v1/auth_token/query/getWanAccessToken?api_key=65296608431940f190e15665063d1bb3&signal=" + signal.toString(), requestOptions)
			...
			.then(function (result)  {
				...
				var body = getencodeBody();//currentSettings.body;
				console.log(body);
				// Can the body be converted to JSON?
				if (body) {
					try {
				...
				$.ajax({
					url: requestURL + currentSettings.deviceId +'/getAppDataByDate?access_token=' + access_token,
					dataType: (errorStage == 1) ? "JSONP" : "JSON",
					type: currentSettings.method || "GET",
					data: body,
					beforeSend: function (xhr) {
	...
	freeboard.loadDatasourcePlugin({
		...
		newInstance: function (settings, newInstanceCallback, updateCallback) {
			newInstanceCallback(new zetaDatasource(settings, updateCallback));
		}
	});
